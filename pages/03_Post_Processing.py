...
with tab3:
    ...
    if st.session_state.data_dlc is not None and st.session_state.neuron_data is not None:
        ...
        if st.session_state.merged_data is not None:
            ...
            st.write(st.session_state.merged_data.df_merged)
    else:
        st.warning(
    ...
    if st.session_state.merged_data is not None:
        ...
        with st.expander("Generate RF Mapping Animation", expanded=False):
            ...
            with col1:
                bending = st.checkbox(
                    "Threshold on Bending", value=False, key="rf_mapping_bending")
                spikes = st.checkbox("Threshold on Spikes",
                                     value=False, key="rf_mapping_spikes")
        ...
        with st.expander("Plot KDE / Scatter", expanded=False):
            ...
            with col1:
                bending = st.checkbox(
                    "Threshold on bending", value=False, key="kde_scatter_bending")
                spikes = st.checkbox("Threshold on spikes",
                                     value=False, key="kde_scatter_spikes")
            ...
            if st.checkbox("Show KDE Density"):
        ...
        with st.expander("Scrolling Video Inputs", expanded=False):
            ...
            scroll_video_file = st.file_uploader("Upload the original video file", type=[
                                                 "mp4", "avi"], key="scroll_video")

            if scroll_video_file is not None:
                st.success(f"Uploaded video file: {scroll_video_file.name}")
                scroll_video_bytes = scroll_video_file.read()  # Just read now, don’t write yet
                ...
                if len(scroll_columns) != 2:
                    ...
                    if st.button("Generate Scrolling Overlay Video"):
                        try:
                            ...
                            # Now write the uploaded video to temp file
                            with NamedTemporaryFile(delete=False, suffix=".mp4") as temp_video_file:
                                temp_video_file.write(scroll_video_bytes)
